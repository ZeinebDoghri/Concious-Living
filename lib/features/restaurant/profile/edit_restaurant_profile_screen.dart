...
class _EditRestaurantProfileScreenState extends State<EditRestaurantProfileScreen>
  ...
  void initState() {
    ...
    _restaurantNameController.text = user?.restaurantName ?? '';

    final cuisine = (user?.cuisineType ?? '').trim();
    if (cuisine.isNotEmpty) {
      _cuisine = _cuisineOptions.contains(cuisine) ? cuisine : 'Other';
    }
    ...
    if (teamSize != null && teamSize > 0) {
      _staffCount = teamSize.clamp(1, 80);
    }
    ...
    if (staffRoles.isNotEmpty) {
      ...
      if (_roles.isEmpty) _roles.add('Chef');
    }
  ...
  void _handleInitialStep() {
    ...
    final stepNumber = int.tryParse(stepParam);
    if (stepNumber == null) return;

    final idx = (stepNumber - 1).clamp(0, 2);
    setState(() {
      _step = idx;
      _prevStep = idx;
    });
...
class _KitchenSafetyStep extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 18),
        Text(
          'Waste threshold: ${wasteThreshold.round()} kg/day',
          style: GoogleFonts.inter(
            fontSize: 13,
            fontWeight: FontWeight.w600,
            color: AppColors.espresso,
